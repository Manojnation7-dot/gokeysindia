...
// SVG Icons... (Keeping your icons as they are)
const CalendarIcon = () => (
...
/**
 * REPLACEMENT FOR DOMParser (Server-Safe)
 */
function extractParagraphTexts(htmlString) {
  if (!htmlString || typeof htmlString !== 'string') return [];

  // Match everything inside <p>...</p> tags
  const pRegex = /<p[^>]*>(.*?)<\/p>/gi;
  const matches = htmlString.match(pRegex);

  if (!matches) {
    // If no <p> tags, remove all other tags and return as one item
    const cleanText = htmlString.replace(/<\/?[^>]+(>|$)/g, "").trim();
    return cleanText ? [cleanText] : [];
  }

  return matches.map(m => 
    m.replace(/<\/?[^>]+(>|$)/g, "").trim() // Strip tags from match
  ).filter(Boolean);
}
              ...
              {(destination.faqs || []).map((faq, index) => {
                ...
                return (
                  <div key={index} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden transition-all">
                    <button onClick={() => toggleDay(index + 10)} className="w-full flex items-center justify-between gap-6 p-6 md:p-8 text-left group">
                      <span className={`text-lg md:text-xl font-bold transition-colors ${isOpen ? "text-indigo-600" : "text-gray-900 group-hover:text-indigo-600"}`}>
                        {faq.question}
                      </span>
                      <div className={`flex items-center justify-center w-10 h-10 rounded-full transition-all ${isOpen ? "bg-indigo-600 text-white rotate-180" : "bg-gray-100 text-gray-500 group-hover:bg-indigo-50"}`}>
      ...
      <section id="tours-section" className="bg-white py-12">
        <div className="max-w-6xl mx-auto px-6">
          ...
          {tours.length > 0 ? (
            ...
            {tours.map((tour) => (
              <div key={tour.id} className="group bg-white rounded-[2rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col">
                <div className="relative h-64 overflow-hidden">
                  <Image src={tour.featured_image?.image || "/images/placeholder.jpg"} alt={tour.name} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-indigo-600 transition-colors">{tour.name}</h3>
                  <p className="text-gray-500 text-sm mb-6 line-clamp-2 leading-relaxed" dangerouslySetInnerHTML={{ __html: tour.content }} />
                  <div className="mt-auto flex items-center justify-between">
                    <div>
                      <span className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-1">Starting from</span>
                      <span className="text-2xl font-extrabold text-indigo-600">₹{parseFloat(tour.base_price || 0).toLocaleString()}</span>
                    </div>
                    <Link href={`/tours/${tour.slug}`}>
                      <button className="px-6 py-3 bg-gray-900 hover:bg-indigo-600 text-white rounded-2xl text-sm font-bold transition shadow-md">Details</button>
                    </Link>
                  </div>
                </div>
              </div>
      ...
      <section id="hotels-section" className="bg-white py-12 border-t border-gray-200">
          <div className="max-w-6xl mx-auto px-6">
            ...
            {hotels.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
              {hotels.map((hotel) => (
                <div key={hotel.id} className="group bg-white rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col">
                  <div className="relative h-56 overflow-hidden">
                    <Image src={hotel.front_image_url || "/images/placeholder.jpg"} alt={hotel.name} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    <h3 className="text-lg font-bold text-gray-900 mb-1 group-hover:text-emerald-600 transition-colors">{hotel.name}</h3>
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      <div>
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-1">Starting from</span>
                        <span className="text-2xl font-extrabold text-emerald-600">₹{parseFloat(hotel.tariff_starting_from).toLocaleString()}</span>
                      </div>
                      <Link href={`/hotels/${slug}/${hotel.slug}`}>
                        <button className="px-6 py-3 bg-gray-900 hover:bg-emerald-600 text-white rounded-2xl text-sm font-bold transition shadow-md">Details</button>
                      </Link>
                    </div>
                  </div>
                </div>
        ...
        <section className="py-12 bg-indigo-50">
          <div className="max-w-4xl mx-auto px-6 text-center">
            ...
            <p className="mt-2 text-gray-600">Need help choosing a tour or want to customize your travel experience?</p>
            <a href="/contact" className="mt-6 inline-block px-6 py-3 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition">Contact Us</a>
          </div>
        </section>
      </main>
      <Footer />
